...
8. **Long text hangs the daemon (infinite CPU spin).** Text longer than ~3000–3500 chars makes the daemon spin forever with no response — the client times out (3×30s socket retries) and Hermes reports "timed out after 60s". ~2000 chars is safe, ~3000 works, ~3500 hangs (threshold is length-dependent, not character-dependent). The daemon gets STUCK after a hang: subsequent requests block behind it, so kill it first (`pkill -x MatchaTTSInfer` + `rm -f /tmp/tts_infer.sock`) before retrying. **Fix:** split long text into chunks ≤ ~2000 chars and call `text_to_speech` once per chunk (delivers as separate voice messages).
