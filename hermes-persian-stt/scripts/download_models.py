#!/usr/bin/env python3
"""
Download model files for hermes-persian-stt.
...
SKILL_DIR = SCRIPT_DIR.parent  # hermes-persian-stt/
...
def download_file(url: str, dest: Path, label: str):
    ...
    req = Request(url, headers={"User-Agent": "hermes-persian-stt/1.0"})
...
def download_hush():
    ...
    req = Request(HUSH_URL, headers={"User-Agent": "hermes-persian-stt/1.0"})
    with urlopen(req) as resp:
...
def main():
    parser = argparse.ArgumentParser(description="Download models for hermes-persian-stt")
